...
(defun initialize-generator (&optional (seed 12345) (state *MT*))
	"initializes the generator from a seed"
	(setf (index state) 0)
	(setf (first (mt-vector state)) seed)
	(loop for i from 1 to 623
		do (setf (nth i (mt-vector state)) ;; MT[i] := lowest 32 bits of(0x6c078965 * (MT[i-1] xor (right shift by 30 bits(MT[i-1]))) + i)
				(logand 
					(*  
						#x6c078965
						(+ 
							(logxor 
								...
								(ash (nth (- i 1) (mt-vector state)) (- 30)))
							i))
					#xFFFFFFFF))))

(defun generate-numbers (&optional (state *MT*))
	(let (y)
		(loop for i from 0 to 623 
			do (setf y 						;; y = (MT[i] and 0x80000000) + (MT[(i+1) mod 624] and 0x7FFFFFFF)
					(+
						(logand (nth i (mt-vector state)) #x8000000)
						(logand (nth (rem i 624) (mt-vector state)) #x7fffffff)))
			...
			when (not (evenp y))			;;When odd
				do	(setf (nth i (mt-vector state))		;; MT[i] = MT[i] xor 0x9908b0df
						(logxor (nth i (mt-vector state)) #x9908b0df)))))


(defun extract-number (&optional (state *MT*))
	"Extract a tempered pseudorandom number based on the index-th value,
	 calling (generate-numbers) every 624 numbers"
	 (if (= (index state) -1)
	 	(initialize-generator (logand (get-universal-time) #xFFFFFFFFFFFFFFFFFF ) state))
	 (if (= (index state) 0)
	 	(generate-numbers))
